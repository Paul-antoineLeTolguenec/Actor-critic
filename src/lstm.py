...
class ActorCritic(tf.keras.Model):
	...
	def __init__(self, num_actions: int):
		...
		self.common_actor_lstm=layers.LSTM(10,activation="softmax",kernel_initializer=initializers.RandomNormal(stddev=0.01),bias_initializer=initializers.Zeros())
		self.common_critic_lstm=layers.LSTM(10,activation="softmax",kernel_initializer=initializers.RandomNormal(stddev=0.01),bias_initializer=initializers.Zeros())
	...
	def call(self, inputs: tf.Tensor):
		x = self.common_actor(self.common_actor_lstm(inputs))
		y = self.common_critic(self.common_critic_lstm(inputs))
...
class Dqn():
	...
	def __init__(self, input_size, nb_action, gamma, n_step=3):
		self.gamma = gamma
		self.n_step=n_step
		...
		self.optimizer = keras.optimizers.RMSprop(learning_rate=0.003)
		...
		self.last_state = np.zeros([n_step, self.input_size])
	...
	def update_states(self, next_state):
		...
		self.last_state[-1] = next_state

	def update(self, reward, new_signal,tape):
		self.update_states( new_signal)
		#convert to tensor
		#self.last_state = np.reshape(self.last_state, [1, self.n_step, self.input_size])
